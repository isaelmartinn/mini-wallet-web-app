import {
  GetTransfersParams,
  GetTransfersUseCase as GetTransfersUseCaseInterface,
...
export class GetTransfersUseCase implements GetTransfersUseCaseInterface {
  ...
  async execute(params: GetTransfersParams): Promise<Transfer[]> {
    const transfers = await this.transferRepository.findByUserId(params.userId);
